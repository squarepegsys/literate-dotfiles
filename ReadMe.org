...
This really uses the magic of OrgMode to generate. In OrgMode, you can put in [[https://orgmode.org/manual/Working-with-source-code.html][code blocks]] and export them to a file, which is called [[https://orgmode.org/manual/tangle.html][tangling]]. OrgMode also [[https://orgmode.org/manual/HTML-export.html][exports to HTML]]. Now there is a makefile to handle this on the command line but it all uses Emacs.
